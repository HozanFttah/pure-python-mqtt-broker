def decode(bytes):
    ...
    #username length
    if decoded_packet["Username flag"]:
        current_byte=current_byte + payload_length_value
        username_length_bytes_1 = bytes[current_byte]
        username_length_bits_1 = format(username_length_bytes_1, "08b")
        current_byte += 1
        username_length_bytes_2 = bytes[current_byte]
        username_length_bits_2 = format(username_length_bytes_2, "08b")
        current_byte += 1
        username_length_bits = username_length_bits_1 + username_length_bits_2
        username_length_value = int(username_length_bits, 2)
        decoded_packet["Username length"] = username_length_value

    #username
        username_bytes = bytes[current_byte: current_byte + username_length_value]
        username = username_bytes.decode()
        decoded_packet["Username"] = username

    #password
    if decoded_packet["Password flag"]:
        current_byte=current_byte + username_length_value+2
        password_bytes = bytes[current_byte:]
        password = password_bytes.decode()
        decoded_packet["Password"] = password
