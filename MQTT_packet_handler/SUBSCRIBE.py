...
def handle(incoming_packet: dict, client_ID: str):
    ...
    for topic in topics:
        ...
        if not MQTT_database.topic_exists(topic_name):
            ...
            if MQTT_database.session_add_topic(client_ID, topic_name):
                print(f'Client ID ({client_ID}) subscribed to ({topic_name})')
                return_codes.append("00000001")   # QoS 1
            else:
                print(f"client_id({client_ID}) already subsicribed in topic ({topic_name})")
                return_codes.append("10000000")   # Failure
